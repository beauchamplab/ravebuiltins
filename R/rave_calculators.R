...
collapse_over_freq_and_trial <- function(el, frange, include_standard_error=TRUE){
    ...
    if(dim(el)[4L] > 1) stop('collapse_over_freq_and_trial Cannot handle #elec > 1')
    ...
    # ~ 159 ms
    eCon <- el$subset(Frequency = (Frequency %within% frange), data_only = TRUE, drop=F)

    #only drop the electrode dimension
    eCon %<>% abind::adrop(drop=4)

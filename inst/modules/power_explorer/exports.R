...
custom_plot_download_renderers <- function(nm) {
    ...
    pest  = function() {
        vars = c("has_data","which_result_to_show_on_electrodes","unit_of_analysis","omnibus_results","mean_filter",
            ...
            "electrodes","show_result_densities","background_plot_color_hint","color_palette", "f1_analysis_settings", "f2_analysis_settings")
    ...
    aotbe = function() {
        f1 <- lapply(local_data$by_electrode_heat_map_data, `[[`, 'F1')
        
        set_heatmap_palette_helper(plot_options=plot_options)
        draw_many_heat_maps(f1,
    ...
    aotbf = function() {
        ...
        decorator <- spectrogram_heatmap_decorator(plot_options = plot_options)
        # if(isTRUE(plot_options$show_stimulation_window)) {
        #     decorator %<>% add_decorator(stimulation_window_decorator)
        # }
        
        hmaps <- lapply(local_data$heat_map_data, `[[`, 'F1')
        draw_many_heat_maps(hmaps = hmaps,
            ...
            PANEL.COLOR_BAR = ifelse(plot_options$show_heatmap_range,
                color_bar_title_decorator, 0)
        )
    }
    aotbt = function() {
        set_heatmap_palette_helper(plot_options=plot_options)
        by_trial_heat_map_data = lapply(local_data$by_trial_heat_map_data, `[[`, 'F1')
    ...
    aotbc = function() {
        time_series_plot(plot_data = lapply(local_data$over_time_data, `[[`, 'F1'),
            plot_time_range = plot_options$plot_time_range,
            PANEL.FIRST = time_series_decorator(plot_options=plot_options)
        )
        
    }
    ptaac = function() {
        sbd <- local_data$scatter_bar_data
        
        fd <- local_data$flat_data
        fd$freq %<>% factor
        ...
        if(nlevels(fd$freq) > 1) {
            pt.loc <- plot_grouped_data(mat = fd, yvar = 'y',
                xvar = 'group_name', gvar='freq', draw0=min(fd$y)<0,
                jitter_seed=cache(name='jitter_seed'))
    ...
    # assign('FF', FUNS, envir = globalenv())
    # assign('nm', nm, envir = globalenv())
    # assign('ld', shiny:::reactiveValuesToList(local_data), envir = globalenv())
    
    # nm = match.arg(nm, names(FUNS))
    
    set_palette_helper(plot_options=plot_options)

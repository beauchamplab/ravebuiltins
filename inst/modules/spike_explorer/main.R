...
if(FALSE) {
  ...
  init_module('spike_explorer', TRUE)
...
over_time_spike_data <- by_trial_spiking_data <- spike_correlation_by_frequency <- scatter_bar_data
...
for(ii in which(has_trials)) {
  ...
  cat2t('Generating spike data')
  spike_mat <- generate_spike_matrix(trials = by_trial_heat_map_data[[ii]]$trials,
                                       times = by_trial_heat_map_data[[ii]]$x,
                                       seed=1000*ii + 143)
  by_trial_spiking_data[[ii]] <- by_trial_heat_map_data[[ii]]
  by_trial_spiking_data[[ii]]$data = spike_mat
  
  cat2t('Building over_time_spike_data')
  over_time_spike_data[[ii]] <- wrap_data(t(
    apply(spike_mat, 1, .fast_mse)),
    xlab='Time (s)', ylab='Spike Density',
    N=dim(power_all_shifted_clean_freq_subset)[4L],
    x=power_all_shifted_clean_freq_subset$dimnames$Time
  )
  
  # set NA (divide by zero) error bars to 0  
  over_time_spike_data[[ii]]$data[is.na(over_time_spike_data[[ii]]$data[,2]),2] <- 0
  
  # we want to make a special range for the line plot data that takes into account mean +/- SE
  over_time_spike_data[[ii]]$range <- .fast_range(plus_minus(over_time_spike_data[[ii]]$data))
  
  spike_correlation_by_frequency[[ii]] <- heat_map_data[[ii]]
  
  
  len = min(50, length(over_time_spike_data[[ii]]$dimnames$Time)/2)
  
  lag_cors <- apply(heat_map_data[[ii]]$data, 2, lagged_cor, x=over_time_spike_data[[ii]]$data[,1], len=len)
  
  lc2 <- apply(heat_map_data[[ii]]$data, 2, lagged_cor, y=over_time_spike_data[[ii]]$data[,1], len=len)

  spike_correlation_by_frequency[[ii]]$data <- rbind(lc2[len:2], lag_cors)
  spike_correlation_by_frequency[[ii]]$x <- -len:(len+1) * .01#seq_len(dim(lag_cors)[1L])
  attr(spike_correlation_by_frequency[[ii]]$data, 'xlab') = 'iEEG Lag (s)'
  attr(spike_correlation_by_frequency[[ii]]$data, 'ylab') = 'Frequency'
  attr(spike_correlation_by_frequency[[ii]]$data, 'zlab') = 'Correlation'
}
...
get_stats_per_electrode <- function(ttypes){
    ...
    rave::lapply_async(electrodes, function(e){
      ...
      if(has_data > 1) {
        ...
        df2$group_f %<>% factor(levels = gnames)
         .lsm <- emmeans::emmeans(lm(y ~ group_f, data=df2), pairwise ~ group_f)
      ...
      # here we want to add in the correlation with the spiking electode (if one exists... how do we enforce this?)
      s = generate_spike_matrix(epoch_data$Condition[epoch_data$Trial %in% trial_numbers], times = el$dimnames$Time)
      SPIKE_analysis_window = c(0,0.3)
      s = colMeans(s[el$dimnames$Time %within% SPIKE_analysis_window,])
      .cor = cor.test(s, trial_means) %$% c(estimate, statistic, p.value)
      res %<>% rbind(matrix(.cor, dimnames = list(c('spike_cor', 't(spike_cor)', 'p(spike_cor)'))))
  ...
  adjusted_p = p.adjust(combined_res['p',], method=get_p.adjust_method(p_filter))
  # print("pval adjustment:")
  # print(m_sd(adjusted_p-combined_res[3,]))
  
  if(has_data > 1) {
    stat_lbls = c('m_', 't_', 'p_')
    tmp = combined_res[7:nrow(combined_res),]
    # add the names for these folks
    rownames(tmp) = c(outer(stat_lbls, gnames, paste0), outer(stat_lbls, lbls, paste0))
    
    combined_res = rbind(combined_res[1:6,], adjusted_p, tmp)
  } else {
    combined_res %<>% rbind(adjusted_p)
  }
  row_adjp = which(rownames(combined_res) == 'adjusted_p')
  rownames(combined_res)[row_adjp] = p_filter
...
module = rave::get_module(module='spike_explorer', package = 'ravebuiltins', local=TRUE)
